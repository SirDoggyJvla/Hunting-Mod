...
Adds the foraging of Hunting Mod.
...
local forages = {
	...
	Tracks_Squirrel = {
		type="HuntingMod.Tracks_Squirrel",
		snowChance = -20,
		rainChance = -20,
		minCount=1,
		maxCount=1,
...
Events.onAddForageDefs.Add(function()
	...
	if SandboxVars.HuntingMod.YearsLater then
		-- robin
	...
	-- not in dev mode, apply the sandbox options
		...
		for _,forage in pairs(forages) do
			...
			local animal_boost = option and SandboxVars.HuntingMod[option.."Forage"]/100 or 1
			for zone,value in pairs(forage.zones) do
				local area = areas[zone]
				local area_boost = area and SandboxVars.HuntingMod[area.."Forage"]/100 or 1
