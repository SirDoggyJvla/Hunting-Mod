...
if not HuntingMod.zoneData then
    ...
    function ISForageIcon:new(_manager, _forageIcon, _zoneData)
        HuntingMod.zoneData = _zoneData

        return original_ISForageIcon_new(self,_manager, _forageIcon, _zoneData)
    end
end

HuntingMod.CreateForageIcon = function(player,square,itemType,animal)
    -- scufffed from vanilla code
    local _zoneData = HuntingMod.zoneData
...
HuntingMod.OnFillWorldObjectContextMenu = function(playerIndex, context, worldObjects, test)
    ...
    local option = context:addOptionOnTop(getText("ContextMenu_HuntingMod_DebugMenu"))
    local subMenu = context:getNew(context)
    context:addSubMenu(option, subMenu)

    option.iconTexture = getTexture("media/UI/HuntingMod_gun.png")

    -- create the subsubmenu to spawn animals
    option = subMenu:addOption(getText("ContextMenu_HuntingMod_SpawnAnimal"))
    ...
    for tracks,animal in pairs(HuntingMod.ForageAnimalTracks) do
        local option = subSubMenu:addOption(getText(animal.name),player,HuntingMod.CreateForageIcon,square,tracks,animal)
